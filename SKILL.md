...
description: Use when solving or teaching an algorithm / coding-test problem and you need to bridge how a human intuitively solves it with how to express it as code (Python, Java, C). Triggers — "머릿속으론 풀리는데 코드가 안 써져", 알고리즘 문제를 코드로 옮기기, 입문자에게 풀이 설명, 사람 풀이 → 코드 번역, off-by-one/무한루프/경계조건이 자꾸 틀릴 때.
---
...
알고리즘에서 진짜 어려운 건 "푸는 것"이 아니라, **사람이 머릿속으로 자연스럽게 하는 일을 기계가 따라 할 수 있는 형태로 옮기는 것**이다. 사람은 패턴 매칭과 "그냥 보면 아는" 도약으로 풀지만, 기계는 모든 단계가 명시적이어야 하고, 머릿속에 담아둔 것이 변수로 꺼내져야 하며, 반복·조건·멈춤이 코드로 적혀야 한다.

**핵심 원리: 코드는 갑자기 튀어나오지 않는다. `말 → 손풀이 → 규칙 → 변수/흐름 → 코드` 순으로 번역된다.** 이 다리의 의미는 언어와 무관하고(같음), 달라지는 건 문법뿐이다.

## When to Use

- 알고리즘 / 코딩테스트 문제를 풀거나 가르칠 때 (시키지 않아도, 풀이를 설명할 땐 이 파이프라인을 적용)
- "사람으로는 풀겠는데 코드로 못 옮기겠다" 는 상황
- off-by-one, 무한 루프, 경계/종료 조건이 자꾸 틀릴 때 (대부분 손풀이·트레이싱 생략이 원인)
- 입문자에게 "이 코드가 왜 이렇게 되는지" 를 설명해야 할 때

**When NOT to use:** 이미 자명한 한 줄짜리, 또는 라이브러리 한 번 호출로 끝나는 일. 번역할 "사람의 풀이"가 없으면 이 스킬은 불필요하다.

## The Translation Pipeline (항상 이 순서로)

1. **사람 말로 다시 쓴다.** 문제를 일상어로 옮기고, 손으로 풀 **작은 구체 예제**를 하나 정한다. (예: 배열 5칸, target 하나)
2. **손으로 풀며 "내가 실제로 한 일"을 관찰한다.** 답을 먼저 구하지 말고, 답에 이르는 *절차*를 본다. 종이에 적어라.
3. **암묵지를 명시화한다.** "가운데를 본다", "큰 걸 고른다" 같은 *그냥 아는* 행동을 정확한 규칙으로 쪼갠다. (가운데 = `(low+high)//2`? 정수 나눗셈? 같은 값이면?)
...
이 표가 4단계(상태 → 변수)의 사전이다. "사람의 표현"을 보고 오른쪽을 떠올려라.
...
더 많은 예제(스택·해시·DP로의 번역): **worked-examples.md** 참고.

## Common Mistakes (이 단계를 건너뛰면 거의 항상 틀린다)

| 실수 | 결과 / 고치는 법 |
|---|---|
| 손풀이 없이 바로 코드 | 경계·종료 조건 틀림. **작은 예제로 손풀이 먼저(2단계).** |
| 암묵지를 그대로 둠 ("가운데를 본다") | mid 계산·정수 나눗셈·off-by-one 누락. **3단계로 규칙화.** |
| 종료 조건 안 정함 | 무한 루프. **"언제 멈추나"를 변수 조건으로 명시(5단계).** |
| 머릿속 상태를 변수로 안 뺌 | 기계는 기억 못 함. **매핑표로 변수/자료구조 도출(4단계).** |
| 트레이싱 생략 | 경계 버그. **"없는 값/맨 끝/빈 입력"을 표로 손 트레이싱(6단계).** |
| 외운 알고리즘으로 점프 | 문제가 살짝 다르면 깨짐. **먼저 이 문제 고유 손풀이로 구조 확인.** |

## Red Flags — STOP

- 문제를 읽자마자 코드부터 타이핑하고 있다 → 1~2단계로 돌아가라.
- "대충 이진 탐색이네" 하고 외운 코드를 붙여넣는다 → 이 문제의 손풀이부터.
- `while` 안에서 `low`/`high`(또는 종료 변수)가 매번 바뀌는지 확인 안 했다 → 무한 루프 위험.
- 예제 하나 손으로 안 돌려보고 "맞다"고 한다 → 트레이싱 전엔 미완성.
