...
알고리즘 / 코딩테스트 문제를 풀거나 가르칠 땐 시키지 않아도 이 파이프라인을 적용한다. off-by-one·무한 루프·경계/종료 조건이 자꾸 틀린다면 원인은 대부분 손풀이·트레이싱 생략이다.
...
이진 탐색이 아닌 문제를 만나면, 6단계 코드를 쓰기 전에 [`worked-examples.md`](worked-examples.md)에서 같은 파이프라인을 스택·해시·DP에 적용한 예제를 읽어라.

## Red Flags — 이 신호가 보이면 STOP, 건너뛴 단계로 돌아간다 (건너뛰면 거의 항상 틀린다)
